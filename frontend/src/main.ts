...
function renderProgressItems(
  data: (Asset | { name: string; current: number; total: number })[],
  containerId: string,
): void {
...
function initManageCharts(): void {
    ...
    charts.bar = new Chart(barCtx, {
      ...
      data: {
        ...
        datasets: [
          {
            label: "閒置中",
            data: state.chartData.idle,
            backgroundColor: "#86efac",
          },
          {
            label: "已預約",
            data: state.chartData.reserved,
            backgroundColor: "#fde047",
          },
          {
            label: "借用中",
            data: state.chartData.borrowed,
            backgroundColor: "#fb923c",
          },
          {
            label: "已逾期",
            data: state.chartData.overdue,
  ...
  if (donutCtx) {
    charts.donut = new Chart(donutCtx, {
      ...
      data: {
        datasets: [
          { data: [14.2, 85.8], backgroundColor: ["#800000", "#e5e7eb"] },
        ],
      },
